...
class Lecturas extends StatefulWidget {
  Lecturas({Key? key}) : super(key: key);

  @override
  State<Lecturas> createState() => _LecturaTablaState();
}

class _LecturaTablaState extends State<Lecturas> {
  ...
  Future<_DataSource> getDataSource() async {
    ...
    print(
        "-----------------------------------------------------------------------4");
    Iterable<dynamic> listred =
        await rutas.map((e) => _Row(e[0], e[1])); //vrificar esta linea
    print(
        "-----------------------------------------------------------------------5");
    listred.forEach((element) {
      print(element);
    });
    ...
    return DataRow.byIndex(
      ...
      onSelectChanged: (bool? value) {
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    LecturaTabla(ruta: row.sector, sector: row.ruta)));
      },
      cells: [
        DataCell(Padding(
